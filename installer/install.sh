...
libpath=$(pwd)/lib
make targets lib/libcsieve.a lib/libcsieve.so
...
if [ -x $(which ldconfig) ]; then
	confdir=/etc/ld.so.conf.d
	if [ -d ${confdir} ]; then
		echo cp ${projpath}/installer/libcsieve.so.conf ${confdir}
		cp ${projpath}/installer/libcsieve.so.conf ${confdir}
	else
		# some systems, like alpine, don't use a config directory
		ldconfig -n $(dirname ${target})
	fi
	ldconfig
  if [ $? != 0 ]; then
    echo error: ldconfig exited with non-zero status $?
  fi
fi

# run sieve library test app
cd ${projpath}/src/main/c/sieve
make test
